...
respace = true; % whether to respace boundary points at every iteraction
includeT = false;
% if we don't include tangential velocity T when propagating boundary
% points, respacing is automatically done so we can just set respace to
% false.
if includeT == false
    respace = false;
end
...
for intIndx = 1:numintervals
    ...
    for tindx = 1:numtsteps
        ...
        if includeT == false
            Znext = GetNextZ_withSurfaceTension(Zcurr2,thetacurr2,Vncurr,Lcurr2,zeta,dt,includeT);
            [Znext,Lnext] = respaceZ(Znext);
        else
            [Znext,Lnext] = GetNextZ_withSurfaceTension(Zcurr2,thetacurr2,Vncurr,Lcurr2,zeta,dt,includeT);
        end
%         [Znext,Lnext,theta_next,kappa_next,UtoNext] = ...
%                 GetNextZ_withSurfaceTension(Zcurr2,thetacurr2,Vncurr,Lcurr2,zeta,dt,includeT);
        % calculate Z at dt/2 later
        if includeT == false
            Zhalf = GetNextZ_withSurfaceTension(Zcurr2,thetacurr2,Vncurr,Lcurr2,zeta,dt/2,includeT);
            [Zhalf,Lhalf] = respaceZ(Zhalf);
        else
            [Zhalf,Lhalf] = GetNextZ_withSurfaceTension(Zcurr2,thetacurr2,Vncurr,Lcurr2,zeta,dt/2,includeT);
        end
%         [Zhalf,Lhalf,theta_half,kappa_half,UtoHalf] = ...
%             GetNextZ_withSurfaceTension(Zcurr2,thetacurr2,Vncurr,Lcurr2,zeta,dt/2,includeT);
        ...
        if includeT == false
            Znext_2s = GetNextZ_withSurfaceTension(Z_half2,theta_half2,Vn_half,L_half2,zeta,dt/2,includeT);
            [Znext_2s,Lnext_2s] = respaceZ(Znext_2s);
        else
            [Znext_2s,Lnext_2s] = GetNextZ_withSurfaceTension(Z_half2,theta_half2,Vn_half,L_half2,zeta,dt/2,includeT);
        end
%         [Znext_2s,Lnext_2s,theta_next_2s,kappa_next_2s] = ...
%             GetNextZ_withSurfaceTension(Z_half2,theta_half2,Vn_half,L_half2,zeta,dt/2,includeT);
